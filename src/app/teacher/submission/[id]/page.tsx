...
export default function SubmissionDetailPage() {
    ...
    if(!submission) return <div className="text-center py-10 dark:text-gray-900">Yükleniyor...</div>;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg shadow">
                <h2 className="font-bold text-xl dark:text-gray-700">{submission.student.name}</h2>
                <p className="text-gray-600 dark:text-gray-400">{submission.homework.book.name} Ödevi</p>
            ...
            {submission.parentNotes && (
                <div className="bg-white p-4 rounded-lg shadow">
                    <h3 className="font-semibold mb-2 dark:text-gray-700">Veli Notu</h3>
            ...
            {submission.photos.length > 0 && (
                <div className="bg-white p-4 rounded-lg shadow">
                    <h3 className="font-semibold mb-2 dark:text-gray-700">Eklenen Fotoğraflar</h3>
            ...
            <div className="bg-white p-4 rounded-lg shadow">
                <h3 className="font-semibold mb-3 flex items-center gap-2 dark:text-gray-700"><ChatBubbleBottomCenterTextIcon className="h-6 w-6 text-indigo-500"/> Yorumlar</h3>
                    ...
                    {submission.comments.map((c: any) => (
                        <div key={c.id} className={`flex gap-2 ${c.user.role === 'teacher' ? 'justify-end' : ''}`}>
                            <div className={`max-w-xs p-3 rounded-lg ${c.user.role === 'teacher' ? 'bg-indigo-500 text-white' : 'bg-gray-200 text-gray-800'}`}>
                                <p className="font-bold text-sm dark:text-gray-700">{c.user.name}</p>
                                <p>{c.commentText}</p>
                ...
                <form onSubmit={handleCommentSubmit} className="mt-4 flex gap-2">
                    <input type="text" value={comment} onChange={e => setComment(e.target.value)} placeholder="Yorumunuzu yazın..." className="mt-1 block w-full px-3 py-2 dark:text-gray-800 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"/>
